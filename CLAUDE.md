...
3. **`DESIGN.md`** — architectural rationale. Update when *why*
   the code does something changes, not just *what*. New decisions /
   future work go in §5.
...
- **Hermetic Python 3.13 via `rules_python`.** Private tools, generated
  live-preview servers, and Python tests are `py_binary` / `py_test` targets
  that request Python 3.13 explicitly. This keeps them independent of the
  consumer's default Bazel-managed Python. The scripts remain stdlib-only;
  `rules_python` supplies the interpreter and Bazel integration, not runtime
  packages. See [DESIGN.md §5 item #11](DESIGN.md).
...
- **Auto-resolved transitive closure.** `resolve_transitive_closure`
  ...
  (see DESIGN.md §5 item #4). To refresh from the pinned index URL:

  ```bash
  bazel run //tools:extract_bundle_manifest -- \
...
- **Tectonic ignores `TEXMFHOME`.** Don't add it — it's a kpathsea
  ...
  item #12.
- **biblatex and biber are a matched pair.** The self-hosted TeX Live 2026
  bundle contains biblatex 3.21 and the toolchain vendors biber 2.21 on every
  supported platform. Modern styles work without a toolchain opt-in; the old
  `modern_biblatex` setting was removed in v0.6.0. Update
  [`latex/private/bundles.bzl`](latex/private/bundles.bzl),
  [`latex/private/biber_versions.bzl`](latex/private/biber_versions.bzl), and
  the bundle manifest together when changing this pair.
...
- **`.github/MAINTAINER_TODO.md`** — punch list for maintainer-only
  follow-ups (currently empty). Not rendered to the docs site or surfaced
...
§4.7 (live preview), §4.9 (biber), and §5 (decisions and future work) are the
