...
Generated by the `latex_live` Bazel rule. Don't edit by hand;
...
def run_bazel_build(
    ...
    if cache_ctx is not None and cache_ctx.module.looks_like_missing_resource(
        ...
        print(
            "latex_live: build failed with what looks like a "
        ...
        try:
            ...
            sys.stderr.write(f"latex_live: re-prime failed: {e}\n")
            sys.stderr.write(result.stderr)
            sys.stderr.flush()
            return (
                False, elapsed, f"BUILD FAILED ({elapsed:.2f}s)",
                combined + f"\nlatex_live: re-prime failed: {e}\n",
            )
        ...
        all_combined = (
            combined
            + "\nlatex_live: re-primed cache; retrying build...\n"
            + retry_combined
        )
        if result.returncode == 0:
            return (
...
<!-- Favicon: the project logo SVG, served from /_assets/logo.svg
     (plumbed through as a runfile by the latex_live rule).
...
def _build_cache_context(workspace: Path, runfiles: Path) -> ServeCacheContext | None:
    ...
    if not serve_cache_path.is_file():
        sys.stderr.write(
            f"latex_live: serve_cache.py missing from runfiles at "
    ...
    if spec is None or spec.loader is None:
        sys.stderr.write(
            f"latex_live: failed to load serve_cache.py from {serve_cache_path}; "
    ...
    if not serve_cache.is_primed(layout):
        try:
            ...
            sys.stderr.write(
                "latex_live: initial serve-cache prime failed "
...
def _build_pdf_chunks_context(
    ...
    if not chunks_lib.is_file():
        sys.stderr.write(
            "latex_live: pdf_chunks.py missing from runfiles at "
    ...
    if spec is None or spec.loader is None:
        sys.stderr.write(
            "latex_live: failed to import pdf_chunks.py; "
    ...
    try:
        ...
        sys.stderr.write(
            f"latex_live: pdf_chunks.py exec failed: {e}; "
            "falling back to whole-PDF transport.\n",
        )
        return None
    ...
    try:
        ...
        sys.stderr.write(
            f"latex_live: could not create chunks dir "
...
def _load_ws_server_module(runfiles: Path) -> object | None:
    ...
    if not ws_lib.is_file():
        sys.stderr.write(
            "latex_live: ws_server.py missing from runfiles at "
    ...
    if spec is None or spec.loader is None:
        sys.stderr.write(
            "latex_live: failed to import ws_server.py; "
    ...
    try:
        ...
        sys.stderr.write(
            f"latex_live: ws_server.py exec failed: {e}; "
            "falling back to SSE for live-reload.\n",
        )
        return None
    return module
...
def _compute_manifest_post_build(
    ...
    try:
        ...
        sys.stderr.write(
            f"latex_live: PDF chunker raised unexpectedly: {e}; "
